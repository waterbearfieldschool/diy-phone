...
constexpr int COLS = 40;                 // chars per row at text size 1
...
void drawStatusBar() {
  ...
  tft.setCursor(SCREEN_W - (int)store.length() * CHAR_W - 2, 3);
...
void drawRead() {
  ...
  int row = bodyText(0, "From " + openSms.sender, ST77XX_MAGENTA);
  Timestamp::Stamp when;
  ...
  if (pos < openSms.body.length()) {
    tft.setCursor(SCREEN_W - 3 * CHAR_W, FOOTER_Y - LINE_H);
    tft.setTextColor(ST77XX_CYAN);
    tft.print("...");
  }
  drawStatusBar();
...
void drawCompose() {
  clearBody();
  drawFooter(screen == Screen::ComposeTo ? "type number  ENT next  ESC cancel"
                                         : "type message  ENT send  ESC cancel");

  int row = bodyText(0, "To: " + composeTo + (screen == Screen::ComposeTo ? "_" : ""),
                     ST77XX_CYAN);
  tft.drawFastHLine(0, BODY_Y + row * LINE_H + 2, SCREEN_W, BAR_BG);

  if (screen == Screen::ComposeBody) {
    bodyText(row + 1, composeBody + "_", ST77XX_WHITE);
    const String count = String(composeBody.length()) + "/" + String(MAX_COMPOSE_CHARS);
    tft.setCursor(SCREEN_W - (int)count.length() * CHAR_W - 2, FOOTER_Y - LINE_H);
    tft.setTextColor(DIM);
    tft.print(count);
  }
  drawStatusBar();
...
void onNotification(const String &line) {
  ...
  if (line.startsWith("+CMTI:")) {
    ...
    loadInbox();
    if (inboxCount > 0) {
      toast("New message from " + inbox[0].sender);
    }
  ...
  switch (screen) {
    ...
    case Screen::Read:
      if (k == Key::Down) { readScroll++; drawRead(); }
        ...
        composeTo = openSms.sender; composeBody = "";
    ...
    case Screen::ComposeTo:
      if (k == Key::Enter && composeTo.length() > 0) { screen = Screen::ComposeBody; drawCompose(); }
