/**************************************************************************
  DIY Phone v21 - SMS Inbox with Auto-Delete SMS + Improved Display
  ...
  v21 Changes: 
  - Replace comma with space in date/time display
  - Re-implement AT+CMGRD (read and delete) for new SMS
 **************************************************************************/
...
GFXcanvas1 inbox_canvas(320, 100);
#define statusY 10
#define inboxY 30
...
void updateInbox() {
  // Clear the actual display area first
  tft.fillRect(0, inboxY, 320, 100, ST77XX_BLACK);
  
  // Clear inbox canvas (for 1-bit canvas, use 0 for black/clear)
  inbox_canvas.fillScreen(0);
  inbox_canvas.setTextSize(1);
  inbox_canvas.setTextColor(1);  // Use 1 for white text in 1-bit canvas
  ...
  tft.drawBitmap(0, inboxY, inbox_canvas.getBuffer(), inbox_canvas.width(), inbox_canvas.height(), ST77XX_WHITE);
