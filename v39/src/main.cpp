...
// v39 Call state variables
bool inCall = false;                   // Track if currently in a call
uint8_t callVolume = 5;                // Current call volume (0-9, default 5)
...
void handleKeyboardV26() {
  ...
  if (Wire.available()) {
    ...
    if (keyData != 0) {
      ...
      // Up/Down arrows - behavior depends on call state and active pane
      else if (keyData == 0xB5) { // UP arrow
        if (inCall) {
          // v39: Increase volume during call
          if (callVolume < 9) {
            callVolume++;
            cellular.setVolume(callVolume);
            char statusText[32];
            snprintf(statusText, sizeof(statusText), "Volume: %d", callVolume);
            updateStatus(statusText, ST77XX_CYAN);
            Serial.print("[CALL] Volume increased to: ");
            Serial.println(callVolume);
          }
        ...
        if (inCall) {
          // v39: Decrease volume during call
          if (callVolume > 0) {
            callVolume--;
            cellular.setVolume(callVolume);
            char statusText[32];
            snprintf(statusText, sizeof(statusText), "Volume: %d", callVolume);
            updateStatus(statusText, ST77XX_CYAN);
            Serial.print("[CALL] Volume decreased to: ");
            Serial.println(callVolume);
          }
        } else if (currentPane == PANE_THREADS) {
          scrollThreadSelection(1);
      ...
      // v38: Capital 'C' initiates a call with selected contact
        ...
        if (selectedThreadIndex >= 0 && selectedThreadIndex < threadPreviewCount) {
          ...
          if (cellular.makeCall(phoneForCall.c_str())) {
            ...
            inCall = true;  // v39: Set call state
          } else {
      ...
      // v39: ESC also hangs up if in a call
      ...
      // v39: 'H' or 'h' hangs up the call if in a call
      else if ((keyData == 'H' || keyData == 'h') && inCall) {
        cellular.hangUp();
        inCall = false;
        updateStatus("Call ended", ST77XX_YELLOW);
