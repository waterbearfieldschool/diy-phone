...
void drawInbox() {
  // The inbox swaps large amounts of text (and an inverted bar) every
  // repaint; partial refreshes leave visible residue. Page-turn cleanly.
  requestFullRefresh();
  clearBody();
  ...
  display.setTextSize(1);
  const bool scrolled = (selected > 0);
  if (scrolled) {
    bigFont();
    display.setTextColor(INK);
  ...
  const int y0 = BODY_Y + (scrolled ? BIG_H : 0);
  const int rowsAvail = BODY_ROWS - (scrolled ? 1 : 0);
  ...
  display.fillRect(0, y0, SCREEN_W, BIG_H, INK);
  display.setTextColor(PAPER);
    ...
    case Screen::Inbox:
      if (k == Key::Up && selected > 0) { selected--; drawInbox(); }
        ...
        if (loadMessage(inbox[selected].slot)) {
          ...
          screen = Screen::Read;
          requestFullRefresh();  // clean page turn into the message
          drawRead();
        }
      }
