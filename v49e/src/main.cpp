...
void drawInbox() {
  ...
  // The selected message is always featured at the top with its full text;
  // the messages after it follow as previews. When there are messages above
  // the selection, a small ^N sits at the right edge of the header bar.
  display.setTextSize(1);
  ...
  // Inverted header bar: this is the cursor.
  display.fillRect(0, BODY_Y, SCREEN_W, BIG_H, INK);
  display.setTextColor(PAPER);
  ...
  if (selected > 0) {
    chromeFont();
    const String up = "^" + String(selected);
    display.setCursor(SCREEN_W - (int)up.length() * CHAR_W - 4, BODY_Y + 5);
    display.print(up);
  }
