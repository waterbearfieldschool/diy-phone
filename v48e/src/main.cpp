...
void drawEntry(int i, int y) {
  ...
  if (sel) display.fillRect(0, y, SCREEN_W, 2 * BIG_H, INK);
...
void drawInbox() {
  ...
  // The newest message shows its full text (up to 4 wrapped lines); the
  // rest are two-row previews. Scrolling down falls back to previews only.
  display.setTextSize(1);

  const int featuredBodyRows =
      max(1, min((int)((inbox[0].preview.length() + COLS - 1) / COLS), 4));
  const int featuredRows = 1 + featuredBodyRows;

  // Keep the selection in the visible window (the window's capacity depends
  // on whether the featured entry is on screen).
  if (selected < inboxTop) inboxTop = selected;
  for (;;) {
    const int visible = (inboxTop == 0)
        ? 1 + (BODY_ROWS - featuredRows) / 2
        : BODY_ROWS / 2;
    if (selected < inboxTop + visible) break;
    inboxTop++;
  }
  ...
  if (inboxTop == 0) {
    ...
    if (sel) display.fillRect(0, BODY_Y, SCREEN_W, featuredRows * BIG_H, INK);
    display.setTextColor(sel ? PAPER : INK);
    ...
    if (!f.unread) bigFont();  // the text stays bold while unread
    for (int r = 0; r < featuredBodyRows; r++) {
      ...
      display.setCursor(2, BODY_Y + (1 + r) * BIG_H + BIG_BASE);
      display.print(chunk);
    }

    const int previews = (BODY_ROWS - featuredRows) / 2;
    for (int k = 0; k < previews; k++) {
      const int i = 1 + k;
      if (i >= inboxCount) break;
      drawEntry(i, BODY_Y + (featuredRows + 2 * k) * BIG_H);
    }
  } else {
    const int visible = BODY_ROWS / 2;
    for (int slot = 0; slot < visible; slot++) {
      const int i = inboxTop + slot;
      if (i >= inboxCount) break;
      drawEntry(i, BODY_Y + slot * 2 * BIG_H);
    }
  }
  chromeFont();
  drawStatusBar();
