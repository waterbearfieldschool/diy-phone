...
boot checklist) stays at the small 6px font. The newest message is featured at the top of the inbox with its full text
(bold name|time header, up to 4 wrapped lines, `...` if longer); the
messages after it are flowing two-row `name|time|preview` entries.
Scrolling down past the window switches to previews only. The name is bold (FreeMonoBold shares FreeMono's cell
