...
#cardkb = i2c.scan()[1]  # should return 95
...
sendee_index = 0
sendee_list = address_book
sendee = sendee_list[0]
audio_alert = True
...
def get_messages():
    stats_area.text='(checking messages...)'
...
def delete_all_messages():
    uart.write(bytes('AT+CMGF=1\r',"ascii"))
    time.sleep(.2)
    uart.write(bytes('AT+CMGD=1,4\r',"ascii"))
    time.sleep(.2)
    data=uart.read(uart.in_waiting).decode()
    print(data)
...
while True:
    ...
    # Check for special keys first using bytearray
    if b not in [LEFT, RIGHT, UP, DOWN, TAB, BACK]:
        c=b.decode()
        if (c != ESC and c != NUL):
            if (c == CR):
                print('\nsending:',instr)
