/**************************************************************************
  DIY Phone v30 - Smartphone-Style Messaging Interface (Clean Interface)
  ...
  v30 Changes (Clean Interface):
  - Removed instruction text from top status bar
  - More screen space for messaging interface
  - Cleaner, minimalist appearance
  - Focus on content over interface clutter
  
  v29 Changes (Visual Conversation Layout):
  - Visual conversation layout with left/indented positioning
  - Text wrapping maintains proper indentation
  - Modern messaging app appearance
  
  v28 Changes (Enhanced Thread Display):
  - Conversation format: 'sender: message time'
  - Smart sender identification and space optimization
  
  v27 Changes (Complete Timezone-Aware Message System):
  - Dual-timestamp storage: UTC for sorting + LocalTime for display
  - Automatic timezone conversion for perfect chronological order
  - Enhanced debug features with timezone conversion details
  - Backward compatible with all existing SMS formats
  - Fixed timezone differences between incoming/outgoing messages
...
#define CONVERSATION_HEIGHT 100      // Space for conversation
...
int conversationScrollOffset = 0;     // For scrolling conversation view
...
// Layout constants for v26 - Thread-based UI (Status + Thread Previews + Conversation + Compose)
#define STATUS_SECTION_Y 0
#define STATUS_SECTION_HEIGHT 12  // v30: Reduced height - removed instruction line
#define THREAD_PREVIEW_Y 12            // v30: Moved up 8px due to reduced status height
#define THREAD_PREVIEW_HEIGHT 108      // v30: Gained 8px height  
#define CONVERSATION_Y 120             // Lower pane for conversation (unchanged)
#define CONVERSATION_HEIGHT 100        // Conversation display area
...
// v26 UI functions
void drawThreadPreviewPane();                      // Upper pane: thread previews
...
void setup(void) {
  ...
  drawThreadPreviewPane();     // Thread previews in middle
  ...
  updateStatusMessage("Ready - v30 Interface", ST77XX_GREEN);
  Serial.println("===============================================");
...
void buildThreadPreviewsFromMessages(SMSMessage* messages, int messageCount) {
  Serial.println("=== Building Thread Previews from Loaded Messages ===");
  ...
  // Group messages by contact and find latest for each thread
  for (int i = 0; i < messageCount && threadPreviewCount < 20; i++) {
    if (i % 5 == 0) {
    ...
    if (contactExists) {
      ...
      if (messages[i].timestampValue > threadPreviews[existingIndex].lastTimestamp) {
        threadPreviews[existingIndex].lastMessageTime = messages[i].time;
        threadPreviews[existingIndex].lastMessagePreview = messages[i].content.substring(0, 25); // Preview length
        threadPreviews[existingIndex].lastTimestamp = messages[i].timestampValue;
      }
...
void calculateOptimalScroll() {
  // Calculate proper scroll position to show complete latest messages above compose line
  ...
  int screenWidth = 320;
  int incomingLeftMargin = 5;
  int outgoingLeftMargin = screenWidth / 3;
  int timeWidth = 30;
  
  // Start from the latest message and work backwards to see what fits
  int totalHeight = 0;
  int messagesFit = 0;
  
  for (int i = currentThreadMessageCount - 1; i >= 0; i--) {
    ...
    // Estimate lines needed for this message (simple character count estimation)
    ...
    if (totalHeight + messageHeight > conversationDisplayHeight) {
      break; // This message won't fit
    }
  ...
  // Set scroll offset to show the messages that fit
  conversationScrollOffset = max(0, currentThreadMessageCount - messagesFit);
  
  Serial.print("Auto-scroll: showing ");
  Serial.print(messagesFit);
  Serial.print(" messages, scroll offset: ");
  Serial.println(conversationScrollOffset);
}

int calculateMaxScrollOffset() {
  // Calculate the maximum valid scroll offset to prevent scrolling past content
  if (currentThreadMessageCount == 0) {
    return 0;
  }
  
  // Simple approach: allow scrolling up to show first message at top
  // More sophisticated approach would calculate based on actual rendered heights
  return max(0, currentThreadMessageCount - 1);
}
...
void addMessageToThread(const String& content, bool isOutgoing, const String& timestamp) {
  ...
  if (currentThreadMessageCount > 8) {
    conversationScrollOffset = currentThreadMessageCount - 8;
  }
...
void scrollThreadSelection(int direction) {
  ...
  int maxVisibleLines = (THREAD_PREVIEW_HEIGHT - 20) / 10;
...
void scrollConversation(int direction) {
  ...
  int tempScrollOffset = conversationScrollOffset; // Save current
  calculateOptimalScroll();
  int optimalPosition = conversationScrollOffset;
  conversationScrollOffset = tempScrollOffset; // Restore current
  
  // If user scrolled to optimal position, resume auto-scrolling
  if (conversationScrollOffset == optimalPosition) {
    userIsManuallyScrolling = false;
    Serial.println("User scrolled back to bottom - resuming auto-scroll");
  }
  
  drawConversationPane();
  Serial.print("Manual scroll offset: ");
  Serial.println(conversationScrollOffset);
}
...
void addCharToCompose(char c) {
  if (composeBuffer.length() < 100) { // Limit compose length
    composeBuffer += c;
    drawConversationPane(); // Refresh to show new character
...
void sendMessage() {
  ...
  drawThreadPreviewPane(); // Refresh thread list
  drawConversationPane();  // Refresh conversation
}

// Old v24 functions removed - replaced with v26 thread-based system

// Old updateInbox function removed - replaced with drawThreadPreviewPane
...
void drawThreadPreviewPane() {
  // Clear thread preview pane area
  tft.fillRect(0, THREAD_PREVIEW_Y, 320, THREAD_PREVIEW_HEIGHT, ST77XX_BLACK);
  
  // Draw header
  tft.setCursor(5, THREAD_PREVIEW_Y + 5);
  tft.setTextColor(ST77XX_CYAN);
  tft.setTextSize(1);
  tft.print("MESSAGES");
  
  // Show thread count
  tft.setCursor(250, THREAD_PREVIEW_Y + 5);
  tft.print(String(threadPreviewCount) + " chats");
  
  // Draw thread previews (10 pixels per line, starting at Y+15)
  int maxLines = (THREAD_PREVIEW_HEIGHT - 20) / 10; // Reserve space for header
  tft.setTextColor(ST77XX_WHITE);
  
  for (int i = 0; i < maxLines && (i + previewScrollOffset) < threadPreviewCount; i++) {
    int threadIndex = i + previewScrollOffset;
    int yPos = THREAD_PREVIEW_Y + 15 + (i * 10);
    
    // Highlight selected thread
    if (threadIndex == selectedThreadIndex) {
    ...
    // Format: "Contact | Time | Preview..." 
    String line = threadPreviews[threadIndex].contactDisplayName.substring(0, 12);
    line += " | ";
    
    // Format timestamp for display
    String timeStr = threadPreviews[threadIndex].lastMessageTime;
    if (timeStr.indexOf(',') != -1) {
      String timePart = timeStr.substring(timeStr.indexOf(',') + 1);
      ...
      line += timeStr.substring(0, 8);
    }
    line += " | ";
    line += threadPreviews[threadIndex].lastMessagePreview.substring(0, 18);
...
void drawConversationPane() {
  ...
  tft.fillRect(0, CONVERSATION_Y, 320, CONVERSATION_HEIGHT, ST77XX_BLACK);
  ...
  // v29: Visual conversation layout with dynamic message spacing
  int conversationDisplayHeight = CONVERSATION_HEIGHT - 20; // Reserve for header
  int screenWidth = 320;
  int incomingLeftMargin = 5;
  ...
  int currentY = CONVERSATION_Y + 15;
  int msgIndex = conversationScrollOffset;
  
  // Draw messages with proper wrapping and indentation
  while (msgIndex < currentThreadMessageCount && currentY < (CONVERSATION_Y + conversationDisplayHeight)) {
    ...
    if (msg.isOutgoing) {
      ...
      int messageMaxWidth = screenWidth - outgoingLeftMargin - timeWidth - 10; // Leave space for timestamp
      
      // Draw message content with wrapping at indented position
      int linesUsed = drawWrappedText(msg.content, messageStartX, currentY, messageMaxWidth, ST77XX_GREEN);
      
      // Draw timestamp at end of first line
      tft.setTextColor(ST77XX_CYAN);
      tft.setCursor(screenWidth - timeWidth, currentY);
      tft.print(displayTime);
      
      currentY += linesUsed * 10; // Move down by number of lines used
      
    } else {
      // v29: Incoming messages - left-aligned at margin, white text
      int messageStartX = incomingLeftMargin;
      int messageMaxWidth = screenWidth - incomingLeftMargin - timeWidth - 10; // Leave space for timestamp
      
      // Draw message content with wrapping at left margin
      int linesUsed = drawWrappedText(msg.content, messageStartX, currentY, messageMaxWidth, ST77XX_WHITE);
    ...
    currentY += 2; // Small gap between messages
    msgIndex++;
  }
  
  // Draw compose area at bottom
  tft.fillRect(0, COMPOSE_Y, 320, COMPOSE_HEIGHT, ST77XX_BLACK);
...
void drawPaneBorder(ActivePane pane) {
  ...
  if (pane == PANE_THREADS) {
    ...
    tft.drawRect(0, THREAD_PREVIEW_Y, 320, THREAD_PREVIEW_HEIGHT, activeBorderColor);
    tft.drawRect(0, CONVERSATION_Y, 320, CONVERSATION_HEIGHT + COMPOSE_HEIGHT, inactiveBorderColor);
  } else {
    // Active conversation, inactive thread previews
    tft.drawRect(0, THREAD_PREVIEW_Y, 320, THREAD_PREVIEW_HEIGHT, inactiveBorderColor);
    tft.drawRect(0, CONVERSATION_Y, 320, CONVERSATION_HEIGHT + COMPOSE_HEIGHT, activeBorderColor);
  }
}
