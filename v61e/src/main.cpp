// main.cpp -- v61e: the DIY phone on a 1.54" e-ink panel (Waveshare rev2.1,
// SSD1681, GxEPD2 vendored in lib/ with two small access patches).
//
// Display strategy, arrived at empirically (see README):
//   * ALL-DRIVEN PARTIALS: each partial refresh writes the frame to the
//     controller's current RAM plane and its INVERSE to the "previous
//     image" plane, so every pixel is driven -- crisp text always, no fade
//     toward the panel's gray partial equilibrium, no ghost accumulation,
//     and no blinking.
//   * Full refreshes exist only at boot and as a rare idle panel-care
//     cycle -- page changes are all-driven partials too, so the phone
//     never blinks during use.
//   * displayWindow() is never used: its window refresh grays the panel.
//
// Screens: Inbox (home), Read, Compose (picker then body), Call, Dial,
// Status. Messages live on the SIM, newest first; unread rows carry a
// right-edge rectangle, the selection a left-gutter dot. M composes
// (resumes drafts), C calls, P calls the selected sender, digits dial.
//
// Serial (115200, local echo): AT passthrough, status, debug, ram, help;
// every panel refresh is logged as [epd].

#include <GxEPD2_BW.h>  // vendored in lib/GxEPD2, two marked access patches
#include <SPI.h>
#include <Wire.h>
...
// The refresh rule: ALL-DRIVEN PARTIALS for everything -- page changes
// included. Since every pixel is driven on every update, full refreshes
// are not needed for image quality at all; the only ones left are boot
// and a rare idle panel-care blink (vendors recommend occasional full
// cycles for the health of the e-ink film itself).
//
...
// Bar-only updates push the whole frame as a normal partial refresh. The
// panel handles full-frame partials cleanly (navigation proves it), but
// displayWindow()'s window refresh visibly grays every undriven pixel --
// confirmed by the [epd] log preceding each gray-out.
void flushBar() { flush(); }
...
// ------------------------------------------------------------------ chrome
// What the status bar currently shows -- updated by drawStatusBar itself,
// so the vitals tick never repaints (and fades the panel) over a bar that
// some full-screen draw already brought up to date.
int shownBars = -2, shownUsed = -1, shownTotal = -1;
int shownUnread = -1, shownMissed = -1;
...
void drawStatusBar() {
  ...
  shownBars = signalBucket(); shownUsed = smsUsed; shownTotal = smsTotal;
...
void drawInbox() {
  ...
  for (int row = 0; row < BODY_ROWS; row++) {
    ...
    // Left column: a dot marks the selected row -- one uniform x position
    // makes the current line easy to find. Right column: a solid rectangle
    // flags unread, bold enough to catch at a glance.
    if (i == selected) display.fillCircle(4, y + 3, 2, INK);
    if (m.unread) display.fillRect(SCREEN_W - 5, y - 1, 5, LINE_H, INK);
...
void drawRead() {
  ...
  display.setTextColor(INK);
  unsigned pos = (unsigned)readScroll * RCOLS;
  for (int ry = y0; ry + RLINE <= FOOTER_Y && pos < openSms.body.length();
       ry += RLINE, pos += RCOLS) {
    display.setCursor(2, ry);
    display.print(openSms.body.substring(pos, pos + RCOLS));
  }
  display.setTextSize(1);
...
void refreshVitals() {
  ...
  // Repaint the bar only when it differs from what is actually on screen.
...
void onNotification(const String &line) {
  ...
  if (line.startsWith("+CMTI:")) {
    if (screen == Screen::ComposeTo || screen == Screen::ComposeBody) {
      ...
      return;             // the new row appears when compose is left
    }
    loadInbox();
    // Refresh the slot count now, so the bar drawn into the full refresh
    // is already accurate and the vitals tick stays silent afterwards.
    int u, t;
    if (modem.smsSlots(u, t)) { smsUsed = u; smsTotal = t; }
    // No banner: the new row simply appears with its unread dot (and the
    // status-bar count ticks up), via one clean full refresh.
...
void loop() {
  ...
  // Panel care: a full refresh cycle every 30 idle minutes, purely for
  // the e-ink film's long-term health -- with all-driven partials it has
  // no visual after-effects, and it never fires while you're active.
  static uint32_t lastActivity = 0;
  static uint32_t lastCareFull = 0;
  if (callState == CallState::None &&
      millis() - lastActivity >= 1800000UL &&
      millis() - lastCareFull >= 1800000UL) {
    lastCareFull = millis();
    LOGI("epd", "panel-care full refresh");
    flush(true);
  }
