/**************************************************************************
  DIY Phone v28 - Smartphone-Style Messaging Interface (Enhanced Thread Display)
  ...
  - New conversation format: 'sender: message time'
  - Outgoing messages show 'me:' as sender
  - Incoming messages show contact name or last 4 digits of phone
  - Smart text width calculation for optimal space usage
  - Maintains color coding (green=outgoing, white=incoming, cyan=time)
...
void setup(void) {
  ...
  updateStatusMessage("Ready - v28 Interface", ST77XX_GREEN);
  Serial.println("===============================================");
...
void drawConversationPane() {
  ...
  int maxConversationLines = conversationDisplayHeight / 10;
  
  tft.setTextSize(1);
  for (int i = 0; i < maxConversationLines && (i + conversationScrollOffset) < currentThreadMessageCount; i++) {
    int msgIndex = i + conversationScrollOffset;
    int yPos = CONVERSATION_Y + 15 + (i * 10);
    
    SMSMessage& msg = currentThreadMessages[msgIndex];
    
    // v28: New format - 'sender: message time'
    ...
    if (msg.isOutgoing) {
      ...
      if (msg.senderDisplayName.length() > 0 && !msg.senderDisplayName.equals(msg.sender)) {
        senderName = msg.senderDisplayName.substring(0, 8); // Limit name length
      } else {
        // Show last 4 digits of phone number
        String phone = msg.sender;
        if (phone.length() > 4) {
          senderName = phone.substring(phone.length() - 4);
        } else {
          senderName = phone;
        }
      }
    ...
    // Display sender name in appropriate color
    tft.print(senderName);
    tft.print(": ");
    
    // Display message content (calculate available space for message)
    int senderWidth = (senderName.length() + 2) * 6; // Approximate character width
    int timeWidth = 30; // Width for "HH:MM" 
    int availableWidth = 315 - senderWidth - timeWidth; // Total width minus sender and time
    int maxChars = availableWidth / 6; // Approximate chars that fit
    
    tft.print(msg.content.substring(0, max(10, maxChars))); // At least 10 chars
