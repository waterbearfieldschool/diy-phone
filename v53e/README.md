...
boot checklist) stays at the small 6px font. The newest message is always featured at the top with its full text (bold
name|time header, up to 4 wrapped lines); the messages after it are packed
one- or two-row previews. DOWN moves an inverted highlight through the
previews -- a small change, done with quiet partial refreshes. Only when
the selection walks off screen does the layout re-flow, with a full
(blinking) refresh; a big "(triangle) N newer" line then marks how many
messages are above the window. The name is bold (FreeMonoBold shares FreeMono's cell
...
`P` dials the highlighted sender from the inbox or an open message. An
incoming call takes over the screen: caller's name (bold) and number,
`ENTER` answers, `ESC` rejects. In a call: `UP`/`DOWN` speaker volume
(0-5), `LEFT`/`RIGHT` mic gain (0-8), `ESC` or `H` hangs up; a MM:SS
readout updates every 10s. Audio uses the SIM7600 board's own amplifier,
speaker jack, and onboard mic. Unanswered calls still land in the missed
list with a toast.
