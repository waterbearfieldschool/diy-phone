...
public:
    ...
    bool setMicGain(uint8_t level = 4);  // AT+CMICGAIN, 0-8

    // Network Functions
    int getSignalQuality();
    bool getNetworkStatus();
    String getNetworkTime();  // Get network time via AT+CCLK?
    ...
    void flushInput();
    String readSMSRaw(uint8_t index);  // Read SMS without expecting OK response
    String readAndDeleteSMS(uint8_t index);  // Read SMS and delete it (AT+CMGRD)
