/**************************************************************************
  DIY Phone v40 - ProMicro nRF52840 Port
  ...
  v40 Changes:
  - Answer incoming calls with ENTER key when ringing
...
// v40 Call state and volume control
bool incomingCallActive = false;      // True when RING detected, false after answer/hangup
bool inActiveCall = false;            // True when in an active call (answered or outgoing)
...
void setup(void) {
  ...
  Serial.println("=== DIY Phone v40 Starting ===");
  ...
  if (sd.begin(sdConfig)) {
    ...
    if (testFile) {
      testFile.println("DIY Phone v40 Test");
      testFile.close();
  ...
  updateStatusMessage("Ready - v40 Interface", ST77XX_GREEN);
  Serial.println("===============================================");
  ...
  Serial.println("C/c = Call selected contact");
  Serial.println("H/h/ESC = Hang up call");
  Serial.println("===============================================");
}
  ...
  while (Serial1.available()) {
    ...
    if (c == '\r') {
      ...
      if (line.length() > 0) {
        ...
        // v40: Check for call ended (NO CARRIER or call disconnect)
...
void handleKeyboardV26() {
  ...
  if (Wire.available()) {
    ...
    if (keyData != 0) {
      ...
      // v40: UP arrow - adjust speaker volume UP
      ...
      // v40: DOWN arrow - adjust speaker volume DOWN
      ...
      // v40: LEFT arrow - adjust mic gain DOWN
      ...
      // v40: RIGHT arrow - adjust mic gain UP
      ...
      // v40: Enter key - answer incoming call, or normal pane actions
        ...
        if (incomingCallActive) {
          ...
          if (cellular.answerCall()) {
            ...
            inActiveCall = true;  // Now in an active call
          } else {
      ...
      // v38: Capital 'C' initiates a call with selected contact
        ...
        if (selectedThreadIndex >= 0 && selectedThreadIndex < threadPreviewCount) {
          ...
          if (cellular.makeCall(phoneForCall.c_str())) {
            ...
            updateStatus("In Call - Use arrows for vol", ST77XX_GREEN);
            inActiveCall = true;  // v40: Set call state
          } else {
      ...
      // v40: ESC also hangs up if in a call
      else if (keyData == 0x1B) { // ESC
        if (inActiveCall) {
          // Hang up the call
          cellular.hangUp();
          inActiveCall = false;
          updateStatus("Call ended", ST77XX_YELLOW);
          Serial.println("[CALL] Call ended by ESC");
        } else if (currentPane == PANE_THREADS && contactSearchBuffer.length() > 0) {
          contactSearchBuffer = "";
          contactSearchMode = 0;
      ...
      // v40: 'H' or 'h' hangs up the call if in a call
      else if ((keyData == 'H' || keyData == 'h') && inActiveCall) {
        cellular.hangUp();
        inActiveCall = false;
        updateStatus("Call ended", ST77XX_YELLOW);
        Serial.println("[CALL] Call ended by H key");
      }
