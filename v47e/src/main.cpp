...
void loadInbox() {
  ...
  while (pos < reply.length() && inboxCount < MAX_LIST) {
    ...
    if (line.startsWith("+CMGL:")) {
      ...
      if (m.preview.length() < 28) {
        if (m.preview.length() > 0) m.preview += ' ';
        m.preview += line.substring(0, 28 - m.preview.length());
      }
    }
  }
...
void drawInbox() {
  ...
  for (int slot = 0; slot < visible; slot++) {
    ...
    // name|time|preview..., flowing across the entry's two rows. Name and
    // time are bold; the preview goes bold too while unread.
