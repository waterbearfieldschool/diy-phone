...
void drawInbox() {
  ...
  for (int slot = 0; slot < visible; slot++) {
    ...
    const int y = BODY_Y + slot * 2 * BIG_H;

    const bool sel = (i == selected);
    if (sel) display.fillRect(0, y, SCREEN_W, 2 * BIG_H, INK);
    display.setTextColor(sel ? PAPER : INK);

    // "*name       14:22" over an indented preview line.
    String line = m.unread ? "*" : " ";
    line += displayName(m.sender).substring(0, 10);
    const String hm = Timestamp::hhmmAt(m.when, localTzMin);
    const String t = (hm.length() > 0) ? hm : String("--:--");
    while (line.length() + t.length() < (unsigned)COLS) line += ' ';
    line += t;
    display.setCursor(2, y + BIG_BASE);
    display.print(line.substring(0, COLS));

    display.setCursor(2, y + BIG_H + BIG_BASE);
    display.print((" " + m.preview).substring(0, COLS));
  }
  chromeFont();
  drawStatusBar();
  flush();
