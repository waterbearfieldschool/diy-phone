/**************************************************************************
  DIY Phone v33 - Contact Search Interface
  ...
  v33 Changes (Contact Search Interface):
  - Expanded conversation panel to 2/3 of display height (139px vs 100px)
  - Redesigned upper panel as searchable contacts (sorted by recent activity)
  - Added letter-based contact search functionality
  - Improved layout: 1/3 contacts, 2/3 conversation
...
void handleKeyboardV26() {
  ...
  if (Wire.available()) {
    ...
    if (keyData != 0) {
      ...
      // v33: Backspace in contacts pane clears search
...
void drawContactsPane() {
  ...
  tft.fillRect(0, CONTACTS_Y, 320, CONTACTS_HEIGHT, ST77XX_BLACK);
  ...
  // Show contact count (filtered or total)
  tft.setCursor(250, CONTACTS_Y + 5);
  int visibleCount = 0;
  for (int i = 0; i < threadPreviewCount; i++) {
    bool matches = false;
    if (contactSearchBuffer.length() == 0) {
  ...
  tft.print(String(visibleCount) + " contacts");
  ...
  for (int threadIndex = 0; threadIndex < threadPreviewCount && visibleContactIndex < maxLines + previewScrollOffset; threadIndex++) {
    ...
    // v33: Format: "Contact | Time | Recent msg..." (contact-focused display)
    String contactName = threadPreviews[threadIndex].contactDisplayName.substring(0, 14);
    ...
    // Format timestamp for display (shorter for more contact name space)
    ...
    if (timeStr.indexOf(',') != -1) {
      ...
      line += timeStr.substring(0, 5);
    }
    line += "| ";
    line += threadPreviews[threadIndex].lastMessagePreview.substring(0, 15);
...
void drawPaneBorder(ActivePane pane) {
  ...
  if (pane == PANE_THREADS) {
    ...
    tft.drawRect(0, CONTACTS_Y, 320, CONTACTS_HEIGHT, activeBorderColor);
    ...
    tft.drawRect(0, CONTACTS_Y, 320, CONTACTS_HEIGHT, inactiveBorderColor);
