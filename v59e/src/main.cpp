...
bool dirtySinceFull = false;  // any partial since the last full refresh
...
void endCall(const char *why) {
  ...
  screen = Screen::Inbox;
  requestFullRefresh();  // come home to a clean slate
  drawInbox();
  toast(why);
}
...
void loop() {
  ...
  // Any partial refresh drops the panel to a slightly gray equilibrium;
  // only a full refresh restores true black. So once interaction pauses,
  // re-push the frame full -- crisp at rest, quiet while pressing.
