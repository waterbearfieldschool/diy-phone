/**************************************************************************
  DIY Phone v21a - SMS with Message Highlighting + Compose Feature
  ...
  v21a Changes: 
  - Highlight top message in inbox
  - 'c' key opens compose screen 
...
int selectedMessageIndex = 0;  // Index of highlighted message

// Compose mode variables
bool composeMode = false;
String composeRecipient = "";
String composeMessage = "";
bool composingRecipient = true;  // true = typing recipient, false = typing message
...
void updateInbox() {
  ...
  for (int i = 0; i < 10 && (i + inboxScrollOffset) < smsInboxCount; i++) {
    ...
    bool isSelected = (smsIndex == selectedMessageIndex);
    
    // Set highlighting for selected message
    if (isSelected) {
...
void handleKeyboard() {
  ...
  if (Wire.available()) {
    ...
    if (keyData != 0) {
      ...
      // Handle compose mode input
      if (composeMode) {
        if (keyData == 0x0D) { // ENTER key
          if (composingRecipient) {
            // Switch from recipient to message
            composingRecipient = false;
            updateComposeScreen();
          } else {
            // Send the message
            sendComposedMessage();
          }
        } else if (keyData == 0x1B) { // ESC key
          exitComposeMode();
        } else if (keyData == 0x08) { // BACKSPACE
          if (composingRecipient && composeRecipient.length() > 0) {
            composeRecipient.remove(composeRecipient.length() - 1);
            updateComposeScreen();
          } else if (!composingRecipient && composeMessage.length() > 0) {
            composeMessage.remove(composeMessage.length() - 1);
            updateComposeScreen();
          }
        } else if (keyData >= 32 && keyData <= 126) { // Printable characters
          if (composingRecipient) {
            composeRecipient += (char)keyData;
            updateComposeScreen();
          } else {
            composeMessage += (char)keyData;
            updateComposeScreen();
          }
        }
      } else {
        // Normal inbox mode
        // Check for 'c' key to enter compose mode
        ...
        // Check for down arrow to scroll inbox or move selection
        else if (keyData == 0xB6) { // DOWN arrow
          if (selectedMessageIndex < smsInboxCount - 1) {
            selectedMessageIndex++;
            // Auto-scroll if selection goes below visible area
            if (selectedMessageIndex >= inboxScrollOffset + 10) {
              inboxScrollOffset++;
            }
            updateInbox();
            Serial.print("Selected message ");
            Serial.println(selectedMessageIndex);
          }
        }
        // Check for up arrow to scroll inbox or move selection
