...
# Message list lines (showing multiple messages in inbox view with wrapping)
msg_line1 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=35)
msg_line2 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=45)  
msg_line3 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=55)
msg_line4 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=65)
msg_line5 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=75)
msg_line6 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=85)
msg_line7 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=95)
msg_line8 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=105)
msg_line9 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=115)
msg_line10 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=125)
msg_line11 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=135)
msg_line12 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=145)
msg_line13 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=155)
msg_line14 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=165)
msg_line15 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=175)
msg_line16 = label.Label(terminalio.FONT, text="", color=0xFFFF00, scale=1, x=5, y=185)
...
msg_lines = [msg_line1, msg_line2, msg_line3, msg_line4, msg_line5, msg_line6, msg_line7, msg_line8, 
             msg_line9, msg_line10, msg_line11, msg_line12, msg_line13, msg_line14, msg_line15, msg_line16]
...
def wrap_message_content(content, max_width=45):
    # Wrap content to fit within the display width
    lines = []
    words = content.split(' ')
    current_line = ""
...
def display_inbox():
    ...
    # Calculate how many lines each message needs
    message_layouts = []
    for sms in sms_list:
        sender = format_sender(sms['sender'])
        timestamp = format_timestamp(sms['time'])
        content_lines = wrap_message_content(sms['content'])
        
        # Each message needs: header line + content lines
        layout = {
            'header': f"{sender} - {timestamp}",
            'content_lines': content_lines,
            'total_lines': 1 + len(content_lines)
        }
        message_layouts.append(layout)
    
    # Adjust scroll offset to keep selected message visible
    display_messages_with_wrapping(sms_list, message_layouts)
    
    status_area.text = f"MSG {selected_message_index + 1}/{len(sms_list)} - UP/DN:nav RIGHT/ENTER:view C:compose N:new D:del"

def display_messages_with_wrapping(sms_list, message_layouts):
    global selected_message_index, inbox_scroll_offset
    ...
    # Start from the scroll offset message
    for msg_idx in range(inbox_scroll_offset, len(sms_list)):
        if current_line_idx >= len(msg_lines):
            break
            
        layout = message_layouts[msg_idx]
        lines_needed = layout['total_lines']
        
        # Check if we have enough lines left to display this message
        if current_line_idx + lines_needed > len(msg_lines):
            break
        
        # Determine if this message is selected
        is_selected = (msg_idx == selected_message_index)
        color = 0x00FF00 if is_selected else 0xFFFF00
        prefix = ">" if is_selected else " "
        
        # Display header line
        msg_lines[current_line_idx].text = f"{prefix}{layout['header']}"
        msg_lines[current_line_idx].color = color
        current_line_idx += 1
        
        # Display content lines with indentation
        for content_line in layout['content_lines']:
            if current_line_idx < len(msg_lines):
                msg_lines[current_line_idx].text = f"  {content_line}"
                msg_lines[current_line_idx].color = color
                current_line_idx += 1
        
        messages_displayed += 1
    
    # Adjust scroll if selected message is not visible
    if selected_message_index < inbox_scroll_offset:
        inbox_scroll_offset = selected_message_index
        display_messages_with_wrapping(sms_list, message_layouts)
    elif selected_message_index >= inbox_scroll_offset + messages_displayed:
        inbox_scroll_offset = max(0, selected_message_index - 1)
        display_messages_with_wrapping(sms_list, message_layouts)

def refresh_inbox_highlighting():
    # For the new wrapped display, just refresh the entire inbox
    # since highlighting affects multiple lines per message
    if current_view == "inbox":
        display_inbox()
