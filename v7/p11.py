...
def display_thread_view():
    ...
    print(f"Thread view: {len(thread_messages)} messages from {contact_name}")
    
    # Display the thread
    refresh_thread_display()

def refresh_thread_display():
    """Refresh the thread view display with full messages and word wrapping"""
    global thread_messages, thread_contact, thread_scroll_offset, thread_selected_index
    
    if not thread_messages:
        inbox_title.text = "THREAD"
        msg_lines[0].text = "No messages in thread"
        return
        
    contact_name = format_sender(thread_contact)
    
    # Update the title at top of screen
    inbox_title.text = f"THREAD: {contact_name} ({len(thread_messages)} msgs)"
    
    # Ensure selected index is valid
    if thread_selected_index >= len(thread_messages):
        thread_selected_index = len(thread_messages) - 1
    elif thread_selected_index < 0:
        thread_selected_index = 0
    ...
    # Calculate how many lines each message takes and display with wrapping
    current_line = 0
    messages_shown = 0
    start_message = thread_scroll_offset
    
    for msg_idx in range(start_message, len(thread_messages)):
        if current_line >= len(msg_lines):
            break
            
        sms = thread_messages[msg_idx]
        timestamp = format_timestamp(sms['time'])
        content = sms['content']
        
        # Create header line for message
        if msg_idx == thread_selected_index:
            header = f">{timestamp}:"
            header_color = 0x00FF00  # Green for selected
        else:
            header = f" {timestamp}:"
            header_color = 0x888888  # Gray for timestamp
        
        # Display timestamp header
        if current_line < len(msg_lines):
            msg_lines[current_line].text = header
            msg_lines[current_line].color = header_color
            current_line += 1
        
        # Word wrap the message content (45 chars per line)
        content_lines = []
        while content:
            if len(content) <= 45:
                ...
                if break_point == -1:
                    break_point = 45
                content_lines.append(content[:break_point])
                content = content[break_point:].strip()
        
        # Display content lines with selection highlighting
        for line in content_lines:
            if current_line >= len(msg_lines):
                break
            if msg_idx == thread_selected_index:
                msg_lines[current_line].text = f" {line}"
                msg_lines[current_line].color = 0x00FF00  # Green for selected
            else:
                msg_lines[current_line].text = f" {line}"
                msg_lines[current_line].color = 0xFFFF00  # Yellow for normal
            current_line += 1
        
        # Add blank line between messages if there's room
        if current_line < len(msg_lines) and msg_idx < len(thread_messages) - 1:
            msg_lines[current_line].text = ""
            current_line += 1
        
        messages_shown += 1
    
    status_area.text = f"THREAD {thread_selected_index + 1}/{len(thread_messages)} - UP/DN:nav ENTER:view R:reply B:back"
...
def reply_to_thread_message():
    """Reply to the selected message in thread view"""
    global compose_message
    if thread_messages and thread_selected_index < len(thread_messages):
        selected_msg = thread_messages[thread_selected_index]
        compose_message = ""
        display_compose(reply_to=selected_msg['sender'])
