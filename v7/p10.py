...
def refresh_thread_display():
    """Refresh the thread view display"""
    ...
    # Adjust scroll offset to keep selected message visible
    if thread_selected_index < thread_scroll_offset:
        thread_scroll_offset = thread_selected_index
    elif thread_selected_index >= thread_scroll_offset + len(msg_lines):
        thread_scroll_offset = thread_selected_index - len(msg_lines) + 1
    ...
    msg_lines[0].text = f"THREAD: {contact_name} ({len(thread_messages)} msgs)"
    msg_lines[0].color = 0x00FF00
    
    # Display messages starting from line 1
    for i in range(1, len(msg_lines)):
        msg_idx = thread_scroll_offset + i - 1
        if msg_idx < len(thread_messages):
            sms = thread_messages[msg_idx]
            timestamp = format_timestamp(sms['time'])
            content = sms['content']
            
            # Truncate long content for thread view
            if len(content) > 35:
                content = content[:35] + "..."
            
            # Highlight selected message
            if msg_idx == thread_selected_index:
                msg_lines[i].text = f">{timestamp} {content}"
                msg_lines[i].color = 0x00FF00  # Green for selected
            else:
                msg_lines[i].text = f" {timestamp} {content}"
                msg_lines[i].color = 0xFFFF00  # Yellow for normal
        else:
            msg_lines[i].text = ""
