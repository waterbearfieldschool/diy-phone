    ...
    for i, line in enumerate(msg_lines):
        ...
        if line_idx < len(cached_inbox_lines):
            ...
            if msg_idx == selected_message_index:
                # Selected message
                if line_type == "header":
                    # Header line with content: show content part in green
                    sender = format_sender(sms_list[msg_idx]['sender'])
                    timestamp = format_timestamp(sms_list[msg_idx]['time'])
                    header_part = f"{sender} {timestamp} "
                    if len(line_text) > len(header_part):
                        # There's content on this line - highlight the whole line green
                        line.color = 0x00FF00  # Green for selected message content
                    else:
                        # Header only line
                        line.color = 0x888888  # Gray for header
                else:  # content line
                    line.color = 0x00FF00  # Green for selected message content
            else:
                # Non-selected messages
                if line_type == "header":
                    # Check if this header line has content
                    sender = format_sender(sms_list[msg_idx]['sender'])
                    timestamp = format_timestamp(sms_list[msg_idx]['time'])
                    header_part = f"{sender} {timestamp} "
                    if len(line_text) > len(header_part):
                        # Mixed line: header + content, use yellow for content
                        line.color = 0xFFFF00  # Yellow for unselected message content
                    else:
                        # Header only
                        line.color = 0x888888  # Gray for header
                else:  # content line
