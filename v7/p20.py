...
def display_call_screen():
    ...
    if call_mode == "contacts":
        ...
        compose_title.text = "CALL - Select Contact:"
        ...
        compose_title.text = "CALL - Enter Number:"
...
def update_call_status_display():
    ...
    if call_status == "dialing":
        compose_title.text = f"CALLING: {call_contact_name}"
        ...
        compose_title.text = f"CALL ACTIVE: {call_contact_name}"
        compose_content.text = f"Connected\nDuration: {call_duration}s"
        ...
        info_area.text = "Returning to inbox..."
...
def process_call_uart_line(line):
    ...
    global call_in_progress, call_status, call_start_time, call_end_countdown
    ...
    if "VOICE CALL: BEGIN" in line_upper:
        ...
        if ":" in line and len(line.split(":")) > 2:
            try:
                ...
                pass
        update_call_status_display()
        call_end_countdown = 30  # Show for 3 seconds (30 * 100ms)
        ...
        if call_status != "ended":  # If we haven't already processed VOICE CALL: END
            call_status = "ended"
            update_call_status_display()
            call_end_countdown = 30  # Show for 3 seconds
...
def end_call_monitoring():
    ...
    global call_in_progress, call_status, call_end_countdown
...
while True:
    ...
    if call_in_progress and call_status == "connected":
        call_start_time += 0.1  # Increment by loop duration
        # Update display every 10 loops (approximately 1 second)
        if int(call_start_time * 10) % 10 == 0:
            update_call_status_display()
