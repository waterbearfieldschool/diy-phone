...
def invalidate_sms_cache():
    global sms_cache_valid, inbox_cache_valid
    sms_cache_valid = False
    inbox_cache_valid = False
...
# Inbox display cache for full message content
cached_inbox_lines = []
inbox_cache_valid = False
inbox_message_line_map = []  # Maps display line index to message index
...
def build_inbox_cache():
    """Build the cached display lines for the inbox with full message content"""
    global cached_inbox_lines, inbox_cache_valid, inbox_message_line_map
    
    sms_list = load_sms_from_sd()
    if not sms_list:
        cached_inbox_lines = []
        inbox_message_line_map = []
        inbox_cache_valid = True
        return
    
    # Build all display lines for the inbox
    cached_inbox_lines = []
    inbox_message_line_map = []  # Maps each display line to its message index
    
    for msg_idx, sms in enumerate(sms_list):
        sender = format_sender(sms['sender'])
        timestamp = format_timestamp(sms['time'])
        content = sms['content']
        
        # Calculate available width for content on first line
        header = f"{sender} {timestamp} "
        header_width = len(header)
        first_line_content_width = 45 - header_width
        continuation_content_width = 45 - 4  # 4 spaces for indent
        ...
        while remaining_content or first_line:
            if first_line:
                # First line: header + beginning of content
                if len(remaining_content) <= first_line_content_width:
                    # Content fits on first line
                    line_text = f"{header}{remaining_content}"
                    cached_inbox_lines.append((line_text, "header", msg_idx))
                    inbox_message_line_map.append(msg_idx)
                    remaining_content = ""
                else:
                    # Content needs wrapping
                    break_point = remaining_content.rfind(' ', 0, first_line_content_width)
                    if break_point == -1:
                        break_point = first_line_content_width
                    line_content = remaining_content[:break_point]
                    line_text = f"{header}{line_content}"
                    cached_inbox_lines.append((line_text, "header", msg_idx))
                    inbox_message_line_map.append(msg_idx)
                ...
                if len(remaining_content) <= continuation_content_width:
                    # Remaining content fits on this line
                    line_text = f"    {remaining_content}"
                    cached_inbox_lines.append((line_text, "content", msg_idx))
                    inbox_message_line_map.append(msg_idx)
                    remaining_content = ""
                else:
                    # Need more wrapping
                    break_point = remaining_content.rfind(' ', 0, continuation_content_width)
                    if break_point == -1:
                        break_point = continuation_content_width
                    line_content = remaining_content[:break_point]
                    line_text = f"    {line_content}"
                    cached_inbox_lines.append((line_text, "content", msg_idx))
                    inbox_message_line_map.append(msg_idx)
                    remaining_content = remaining_content[break_point:].strip()
    
    inbox_cache_valid = True
    print(f"Inbox cache built: {len(cached_inbox_lines)} lines for {len(sms_list)} messages")

def calculate_inbox_scroll_for_selection():
    """Calculate scroll offset to show the selected message in inbox"""
    global inbox_scroll_offset, selected_message_index
    
    if not inbox_message_line_map or selected_message_index >= len(load_sms_from_sd()):
        return
    
    # Find the first line of the selected message
    selected_line = None
    for line_idx, msg_idx in enumerate(inbox_message_line_map):
        if msg_idx == selected_message_index:
            selected_line = line_idx
            break
    
    if selected_line is not None:
        # Adjust scroll to show selected message, preferably near top of screen
        inbox_scroll_offset = max(0, selected_line)
        
        # Ensure we don't scroll past the end
        max_scroll = max(0, len(cached_inbox_lines) - len(msg_lines))
        inbox_scroll_offset = min(inbox_scroll_offset, max_scroll)

def display_inbox():
    global selected_message_index, inbox_scroll_offset
    hide_all_views()
    
    # Reset title to INBOX
    ...
    # Build inbox cache if needed
    if not inbox_cache_valid:
        build_inbox_cache()
    
    # Calculate scroll position for selected message
    calculate_inbox_scroll_for_selection()
    
    # Display inbox using cached content
    refresh_inbox_display()

def refresh_inbox_display():
    """Refresh the inbox display using cached lines with message highlighting"""
    global selected_message_index, inbox_scroll_offset
    
    # Clear all message lines
    for line in msg_lines:
        line.text = ""
        line.color = 0xFFFFFF  # Default to white
    
    if not cached_inbox_lines:
        msg_lines[0].text = "No messages"
        return
    ...
    for i, line in enumerate(msg_lines):
        line_idx = inbox_scroll_offset + i
        if line_idx < len(cached_inbox_lines):
            line_data = cached_inbox_lines[line_idx]
            line_text, line_type, msg_idx = line_data
            
            line.text = line_text
            
            # Color coding based on selection and line type
            if msg_idx == selected_message_index:
                # Selected message
                if line_type == "header_only":
                    line.color = 0x0000FF  # Blue for selected timestamp
                else:  # content line
                    line.color = 0x0000FF  # Blue for selected message content
            else:
                # Non-selected messages
                if line_type == "header_only":
                    line.color = 0x00FF00  # Green for unselected header
                else:  # content line
                    line.color = 0xFFFFFF  # White for message content
        else:
            line.text = ""
...
def refresh_inbox_highlighting():
    """Refresh inbox highlighting efficiently"""
    ...
    # Rebuild cache if needed and refresh display
    if not inbox_cache_valid:
        build_inbox_cache()
...
def find_message_line_range(msg_idx):
    """Find the first and last line indices for a given message"""
    first_line = None
    last_line = None
    
    for line_idx, mapped_msg_idx in enumerate(inbox_message_line_map):
        if mapped_msg_idx == msg_idx:
...
def is_message_visible(msg_idx):
    """Check if a message is currently visible on screen"""
    first_line, last_line = find_message_line_range(msg_idx)
    if first_line is None or last_line is None:
        return False
    
    # Check if message lines are within visible range
    visible_start = inbox_scroll_offset
    visible_end = inbox_scroll_offset + len(msg_lines) - 1
    
    return first_line >= visible_start and last_line <= visible_end

def scroll_inbox_up():
    global selected_message_index, inbox_scroll_offset
    if selected_message_index > 0:
        next_message_index = selected_message_index - 1
        
        # Check if the next message is visible
        if is_message_visible(next_message_index):
            # Next message is visible, just change selection without scrolling
            selected_message_index = next_message_index
            update_inbox_highlighting_only()  # Only update colors, no redraw
        else:
            # Next message is off-screen, do a page scroll
            selected_message_index = next_message_index
            
            # Find the first line of the new selected message
            first_line, _ = find_message_line_range(selected_message_index)
            if first_line is not None:
                # Scroll to show this message at top of screen
                inbox_scroll_offset = max(0, first_line)
...
def scroll_inbox_down():
    ...
    if sms_list and selected_message_index < len(sms_list) - 1:
        ...
        if is_message_visible(next_message_index):
            ...
            # Find the lines of the new selected message
            first_line, last_line = find_message_line_range(selected_message_index)
            if first_line is not None and last_line is not None:
                # Calculate scroll position to show entire message
                # Try to show message near top, but ensure it fits on screen
                target_scroll = first_line
                
                # If message is too long for screen, show as much as possible
                message_height = last_line - first_line + 1
                if message_height > len(msg_lines):
                    # Message longer than screen, show from the top
                    inbox_scroll_offset = first_line
                else:
                    # Message fits, position it nicely on screen
                    # Try to show a bit above if possible
                    target_scroll = max(0, first_line - 1)
                    
                    # Ensure we don't scroll past the end
                    max_scroll = max(0, len(cached_inbox_lines) - len(msg_lines))
                    inbox_scroll_offset = min(target_scroll, max_scroll)
...
def display_thread_view():
    ...
    thread_messages.sort(key=lambda x: x['filename'])
    ...
    # Calculate initial scroll position to show selected message
